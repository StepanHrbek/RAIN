...
#include "file.h"
 ...
 // extension for playing files in files
 int rfSuperStart=0; // start of your file in WAD-like file
 int rfSuperSize =0; // size of your file
 int rfSuperXor  =0; // readen data will be xored
...
int CALLING fileOpen(char *fileName, int openMode, fileHandle *file)
{
    ...
    *file = hdl;
    ...
    rfSeek((*file)->rf, rfSuperStart, rfSeekAbsolute);
...
int CALLING fileGetSize(fileHandle file, long *fileSize)
{
    ...
    if ( rfSuperSize ) *fileSize=rfSuperSize; else
...
int CALLING fileSeek(fileHandle file, long newPosition, int seekMode)
{
    ...
    switch ( seekMode )
    {
        ...
        case fileSeekEnd:           /* seek from end of file */
            ...
            {
                rfMode = rfSeekAbsolute;
                newPosition+=rfSuperStart+rfSuperSize;
            }
            break;
