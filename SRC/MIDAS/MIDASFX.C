 ...
 *
 * $Id: midasfx.c,v 1.7 1997/07/31 10:56:48 pekangas Exp $
 *
                             ...
                             unsigned *sampleHandle)
{
    ...
    /* Open the sound effect file: */
    if ( (error = mWaveOpen(fileName, &s)) != OK )
        PASSERROR(ID_fxLoadWaveSample);

    /* Allocate sample loading buffer: */
    if ( (error = memAlloc(s->dataLength, (void**) &buffer)) != OK )
    {
        mWaveClose(s);
        PASSERROR(ID_fxLoadRawSample);
    }
    ...
    /* Read the sample: */
    if ( (error = fileRead(s->file, buffer, s->dataLength)) != OK )
    {
        mWaveClose(s);
        memFree(buffer);
        PASSERROR(ID_fxLoadRawSample);
    ...
    smp.sampleLength = s->dataLength;

    lastWAVfreq = s->sampleRate;

    /* Close the sample file: */
    if ( (error = mWaveClose(s)) != OK )
    {
        memFree(buffer);
        PASSERROR(ID_fxLoadRawSample);
    }
...
int CALLING fxGetSamplePlayStatus(unsigned playHandle, unsigned *status)
{
    ...
    *status = fxSampleStopped;

    /* Find the channel number for the sample, if not found the sample is
       not being played: */
    if ( (chan = FindChannel(playHandle)) == NOCHANNEL )
        return OK;

    /* Get channel playback rate - if it is 0, the sound is stopped: */
    if ( (error = SD->GetRate(chan, &rate)) != OK )
        PASSERROR(ID_fxGetSampleStatus);
    if ( !rate )
        return OK;

    *status = fxSamplePlaying;
...
/*
 * $Log: midasfx.c,v $
 * Revision 1.7  1997/07/31 10:56:48  pekangas
 * Renamed from MIDAS Sound System to MIDAS Digital Audio System
 *
 * Revision 1.6  1997/07/29 16:51:10  pekangas
 * Added sample playing status query
 *
 * Revision 1.5  1997/05/20 20:37:59  pekangas
